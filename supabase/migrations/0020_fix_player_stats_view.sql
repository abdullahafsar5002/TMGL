...
SELECT 
    ...
    (SELECT AVG(differential) FROM public.score_differentials WHERE player_id = p.id) AS avg_differential
FROM 
    public.profiles p
LEFT JOIN 
    public.scorecards sc ON p.id = sc.player_id AND sc.status = 'verified'
GROUP BY 
    p.id, p.full_name, p.handicap_index;
